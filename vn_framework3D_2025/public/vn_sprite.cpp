...
void vnSprite::setVertexPosition()
{
	...
	if (pParent != NULL)
	{
		parentX = pParent->posX;
		parentY = pParent->posY;
	}
	...
	//回転
	float v0x_r = v0x * cosf(rot) - v0y * sinf(rot);
	float v0y_r = v0y * cosf(rot) + v0x * sinf(rot);

	float v1x_r = v1x * cosf(rot) - v1y * sinf(rot);
	float v1y_r = v1y * cosf(rot) + v1x * sinf(rot);

	float v2x_r = v2x * cosf(rot) - v2y * sinf(rot);
	float v2y_r = v2y * cosf(rot) + v2x * sinf(rot);

	float v3x_r = v3x * cosf(rot) - v3y * sinf(rot);
	float v3y_r = v3y * cosf(rot) + v3x * sinf(rot);

	// 親座標を加算
	vtx[0].x = v0x_r + posX + parentX;
	vtx[0].y = v0y_r + posY + parentY;

	vtx[1].x = v1x_r + posX + parentX;
	vtx[1].y = v1y_r + posY + parentY;

	vtx[2].x = v2x_r + posX + parentX;
	vtx[2].y = v2y_r + posY + parentY;

	vtx[3].x = v3x_r + posX + parentX;
	vtx[3].y = v3y_r + posY + parentY;
}

//---- setter関数 ----

void vnSprite::setParent(vnSprite* p)
{
	pParent = p;
}
...
//透明度の設定
void vnSprite::setAlpha(float Vel)
{
	vtx[0].a = Vel;
	vtx[1].a = Vel;
	vtx[2].a = Vel;
...
//幅の設定
void vnSprite::setWidth(float Vel)
{
	sizeX = Vel;
}

void vnSprite::setUVRange(float left, float right)
{
...
void vnSprite::addWidth(float Vel)
{
	sizeX = sizeX + Vel;
}

void vnSprite::addAlpha(float Vel)
{
	vtx[0].a = vtx[0].a + Vel;
	vtx[1].a = vtx[1].a + Vel;
	vtx[2].a = vtx[2].a + Vel;
