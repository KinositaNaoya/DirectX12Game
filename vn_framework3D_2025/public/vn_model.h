...
public:
	...
	//モデルデータ(vnmファイル)の情報の取得: const = (この関数内では中身を書き換えない)
	...
	vnModel_MeshData *getMesh()			const;//メッシュデータ(先頭アドレス)の取得
	vnVertex3D *getVertex()				const;//頂点データ(先頭アドレス)の取得
	LPWORD getIndex()					const;//インデックスデータ(先頭アドレス)の取得
