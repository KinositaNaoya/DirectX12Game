...
private:
	...
	//カメラの極座標の情報
	static float radius;	//半径
	static float theta;		//角度(平面角 / 経度)
	static float phi;		//角度(仰角 / 緯度)
...
public:
	...
	static void setTheta(float t);
	static void setPhi(float p);
	...
	//カメラコントロール
	static void FreeCamCtrl();
	//static void ParentCamCtrl(vnObject *pObject);
};